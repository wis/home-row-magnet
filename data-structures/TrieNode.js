...
module.exports = class TrieNode {
   ...
   */
  constructor(character, value = undefined, isCompleteWord = false) {
  ...
  *[Symbol.iterator]() {
    yield this.value;
    const childskeys = this.children.getKeys();
    if (childskeys.length > 0) {
      for (let ck of childskeys) {
        yield* this.children.get(ck);
      }
    }
  }
   ...
   * @return {TrieNode}
   */
  addChild(character, value = null, isCompleteWord = false) {
    if (!this.children.has(character)) {
      this.children.set(
        character,
        new TrieNode(character, value, isCompleteWord)
      );
    }
   ...
   */
  addChildNode(child) {
    if (!this.children.has(child.character)) {
      this.children.set(
        child.character,
        child);
    }

    const childNode = this.children.get(child.character);
